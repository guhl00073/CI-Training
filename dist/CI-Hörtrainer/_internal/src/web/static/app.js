...
async function setLanguage(lang) {
  ...
  setStatus(`Übungssprache gewechselt: ${lang === 'en' ? 'Englisch 🇬🇧/🇺🇸' : 'Deutsch 🇩🇪'}`);
...
// Dynamic Button State Helper (Start -> Audio wiederholen)
function setPlayBtnState(btnId, hasPlayed, repeatText = "Audio wiederholen") {
...
function updateCategoryDatalist(modType = null) {
  ...
  const items = (exercises && exercises[modType]) ? exercises[modType] : [];
  const cats = new Set();
  items.forEach(item => {
    const c = item.category || item.type;
    if (c && c.trim()) cats.add(c.trim());
  ...
  datalist.innerHTML = Array.from(cats).sort().map(cat => `<option value="${escapeHtml(cat)}">`).join("");
}
...
function nextMPItem(userTriggered = false) {
  ...
  setPlayBtnState("mpPlayBtn", false, "Audio wiederholen");
...
function playMPAudio() {
  setPlayBtnState("mpPlayBtn", true, "Audio wiederholen");
...
function renderCategoryManagerList() {
  ...
  rawList.forEach(item => {
    ...
    catMap[cat].total++;
    if (isCustomEntry(item, currentEditorView)) catMap[cat].custom++;
  });
    ...
    return `
      <div class="cat-manager-row">
        ...
        <div class="cat-manager-actions">
          ...
          ${info.custom > 0 ? `<button class="btn btn-danger btn-sm" onclick="promptDeleteCategory('${escapeHtml(cat)}', ${isPureCustom})">🗑️ Löschen</button>` : ''}
        </div>
      </div>
    `;
  }).join("");
...
function initExportImport() {
  const exportBtn = document.getElementById("exportExercisesBtn");
  const importBtn = document.getElementById("importExercisesBtn");
  const fileInput = document.getElementById("importFileInput");

  if (exportBtn) {
    exportBtn.addEventListener("click", () => exportExercisesAsJSON());
  }
...
function initKeyboardShortcuts() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (key === "h" || e.key === "?" || e.code === "F1" || e.key === "F1") {
      e.preventDefault();
      toggleHelpModal();
      return;
    }
    ...
    if (key === "n" || e.code === "KeyN" || e.key === "ArrowRight" || e.code === "ArrowRight") {
      e.preventDefault();
      nextExerciseItem();
      return;
    }
...
function initAutostartStopButtons() {
  document.addEventListener("click", (e) => {
    const stopBtn = e.target.closest(".btn-stop-autostart, .stop-autostart-btn, #mpStopBtn, #esActionStopBtn, #esStopBtn, #msActionStopBtn, #msStopBtn, #numActionStopBtn, #numStopBtn, #sentStopBtn, #sentFullStopBtn, #weaknessStopBtn, #weaknessInputStopBtn, #noiseInputStopBtn");
    if (stopBtn) {
      e.preventDefault();
      stopAutostartAndAudio();
    }
      ...
      <div class="therapist-report-sheet">
        ...
        <div style="border-bottom: 2px solid #3B82F6; padding-bottom: 1rem; margin-bottom: 1.2rem; display: flex; justify-content: space-between; align-items: flex-start;">
          ...
          <div style="text-align: right; font-size: 0.8rem; color: #64748B;">
            ...
            <div>Software: <strong>CI-Hörtrainer v1.0.0</strong></div>
          </div>
        </div>
