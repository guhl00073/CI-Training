...
def seed_minimal_pairs(cursor, items: list, force: bool):
    if force:
        cursor.execute("DELETE FROM exercises_minimal_pairs")
...
def seed_monosyllables(cursor, items: list, force: bool):
    if force:
        cursor.execute("DELETE FROM exercises_monosyllables")
    for idx, item in enumerate(items):
        item_id = item.get("id") or f"mo_{idx+1}_{uuid.uuid4().hex[:4]}"
        cursor.execute("""
            INSERT OR IGNORE INTO exercises_monosyllables (id, word, category, source, difficulty)
            VALUES (?, ?, ?, ?, ?)
        """, (
            item_id,
            item.get("word", ""),
...
def seed_numbers(cursor, items: list, force: bool):
    ...
    for idx, item in enumerate(items):
        ...
        cursor.execute("""
            INSERT OR IGNORE INTO exercises_numbers (id, type, source, value, spoken, difficulty)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            item_id,
...
def main():
    ...
    for key, seeder_fn in seeders.items():
        ...
        if count_before > 0 and not args.force:
